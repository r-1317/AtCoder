...
# 各マスの評価値を計算する関数（task_a.md の AI と同等）
def evaluate(
  ...
  """task_a.md の「AIの行動決定方法」と同等の評価値 A_{p,i,j} を返す。

  各マス (i,j) のプレイヤー player にとっての評価値は以下:
  - 無人 (owner == -1): V[i][j] * wa
  - 自領で L < U:        V[i][j] * wb
  - 自領で L == U:       0
  - 他領で L == 1:       V[i][j] * wc
  ...
  この実装では wa,w b,wc,wd にそれぞれグローバル定数 A,B,C,D を用いる。

  - owners: O[i][j] (-1: 無人, 0..M-1: 所有者)
  - levels: L[i][j]
  - values: V[i][j]
  - M: プレイヤー数（範囲チェック用。盤面計算自体は N に依存）
  - U: レベル上限
  - player: 評価するプレイヤー番号
  """

  if not (0 <= player < M):
    raise ValueError("player must be in [0, M)")

  wa, wb, wc, wd = A, B, C, D
  evals = [[0.0] * N for _ in range(N)]
  ...
  for i in range(N):
    for j in range(N):
      ...
      if o == -1:
        ...
        evals[i][j] = 0.0 if l >= U else v * wb
      else:
        evals[i][j] = v * (wc if l == 1 else wd)

  return evals

def get_random_move(current_positions: List[Tuple[int, int]], owners: List[List[int]], player: int) -> Tuple[int, int]:
  """合法な移動先候補からランダムに1つ選んで返す。"""
  candidates = get_next_positions(current_positions, owners, player)
  if not candidates:
    raise ValueError("No valid moves available")
  return random.choice(candidates)

def get_best_pos(candidates: List[Tuple[int, int]], evals: List[List[float]]) -> Tuple[int, int]:
  """candidates の中で評価値 evals[i][j] が最大の座標を返す。複数ある場合は任意に1つ選ぶ。
  """
  best_pos = None
  best_eval = float('-inf')
  for x, y in candidates:
    if evals[x][y] > best_eval:
      best_eval = evals[x][y]
      best_pos = (x, y)
  return best_pos
...
def main():
  ...
  # 評価値に基づいて次の移動先を選択するループ
  for turn in range(T):
    if random.random() < Epsilon:
      # ランダムに移動
      best_pos = get_random_move(positions, owners, 0)
    else:
      # 評価値に基づいて移動
      evals = evaluate(owners, levels, values, M, U, player=0)
      candidates = get_next_positions(positions, owners, 0)

      best_pos = get_best_pos(candidates, evals)
