...
def evaluate_mask(mask, candidates, force_indices):
	...
	moved = 0
	has_forced = False
	for idx, (i, j, payload) in enumerate(candidates):
		if (mask >> idx) & 1:
			pairs.append((i, j))
			moved += payload[3]
...
def main() -> None:
	...
	# 1) Same as a01/a02/a03: departure i -> siding i, all in one turn.
	first_turn = []
	for i in range(r):
		k = len(departures[i])
	...
	while True:
		...
		for j in range(r):
			if sidings[j]:
				i = sidings[j][0] // 10
				k = 1
				if len(sidings[j]) >= 2 and sidings[j][1] // 10 == i:
					k = 2
				candidates.append((i, j, (1, i, j, k)))
		...
		for _, i, j, k in ops:
			moved = sidings[j][:k]
			sidings[j] = sidings[j][k:]
			departures[i].extend(moved)
	...
	while True:
		...
		for i in range(r):
			if departures[i]:
				j = departures[i][-1] % 10
				k = 1
				if len(departures[i]) >= 2 and departures[i][-2] % 10 == j:
					k = 2
				candidates.append((i, j, (0, i, j, k)))
