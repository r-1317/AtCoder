...
def nearest_valid(pos: Tuple[int, int], used: List[List[bool]]) -> Tuple[int, int]:  # 最寄りの未使用マスを探す
  x, y = pos
  min_dist = float("inf")
...
def main():
  ...
  is_captured = [False] * (N**2)  # 各数字が1枚でも回収済みかどうか
  ...
  for _ in range(N**2//2):
    # 最寄りの未使用マスを探す
    start = nearest_valid(current_pos, used)
