...
bool betterEval(const Eval& a, const Eval& b) {
    ...
    if (a.pressCount != b.pressCount) {
        return a.pressCount > b.pressCount;
    }

    return a.T > b.T;
}

double annealValue(const Eval& e) {
    if (!e.reachable) return -1e100;

    // 主目的: スイッチ押下回数
    // 副目的: 最短行動回数 T
    return e.pressCount * 1000.0 + e.T * 0.001;
}
...
Solution makeInitialSolution(const vector<pair<int, int>>& path) {
    ...
    while ((int)sol.doors.size() < M && guard < 200000) {
        ...
        const vector<Edge>& pool =
            (!openEdges.empty() && rng.nextInt(100) < 85)
            ? openEdges
            : allEdges;
        ...
        // 初期閉扉を多めにする。
        int parity = rng.nextInt(100) < 80 ? 1 : 0;
    ...
    for (const Edge& e : allEdges) {
        ...
        Door door{e.d, e.i, e.j, rng.nextInt(2 * K)};
        sol.doors.push_back(door);
        usedDoor[code] = 1;
    }
...
Eval calcEval(const Solution& sol) {
    ...
    static int bestPress[1 << MAXK][MAXN][MAXN];

    static int que[MAXSTATE];
    static int order[MAXSTATE];
    ...
    while (head < tail) {
        ...
        for (int dir = 0; dir < 4; dir++) {
            ...
            if (seen[mask][ni][nj] != stamp) {
                ...
                dist[mask][ni][nj] = dcur + 1;
                bestPress[mask][ni][nj] = -1;

                int nv = mask * NN + ni * N + nj;
                que[tail++] = nv;
                order[orderSize++] = nv;
            }
        }
        ...
        if (s != -1) {
            ...
            if (seen[nmask][i][j] != stamp) {
                ...
                dist[nmask][i][j] = dcur + 1;
                bestPress[nmask][i][j] = -1;

                int nv = nmask * NN + i * N + j;
                que[tail++] = nv;
                order[orderSize++] = nv;
            }
        }
    ...
    // 次に、最短距離 DAG 上で「スイッチ押下回数の最大値」を DP する。
    bestPress[0][0][0] = 0;
    ...
    for (int idx = 0; idx < orderSize; idx++) {
        ...
        int curPress = bestPress[mask][i][j];

        if (curPress < 0) continue;
        ...
        for (int dir = 0; dir < 4; dir++) {
            ...
            if (seen[mask][ni][nj] == stamp &&
                dist[mask][ni][nj] == dcur + 1) {
                bestPress[mask][ni][nj] =
                    max(bestPress[mask][ni][nj], curPress);
            }
        }
        ...
        if (s != -1) {
            ...
            if (seen[nmask][i][j] == stamp &&
                dist[nmask][i][j] == dcur + 1) {
                bestPress[nmask][i][j] =
                    max(bestPress[nmask][i][j], curPress + 1);
            }
        }
    }
    ...
    for (int mask = 0; mask < (1 << K); mask++) {
        if (seen[mask][N - 1][N - 1] != stamp) continue;

        int d = dist[mask][N - 1][N - 1];
        int p = bestPress[mask][N - 1][N - 1];
...
void swapSwitchPositions(Solution& sol) {
    int a = rng.nextInt(K);
    int b = rng.nextInt(K);
...
int main() {
    ...
    vector<pair<int, int>> path = findInitialPath();

    Solution cur = makeInitialSolution(path);
    Eval curEval = calcEval(cur);

    if (!curEval.reachable) {
        for (auto& e : cur.doors) {
            e.g = 0;
        }
        curEval = calcEval(cur);
    }
    ...
    const double startTemp = 1500.0;
    const double endTemp = 3.0;
    ...
    while (timer.elapsed() < TIME_LIMIT) {
        ...
        Eval nxtEval = calcEval(nxt);

        if (!nxtEval.reachable) {
            continue;
        }

        double curVal = annealValue(curEval);
        double nxtVal = annealValue(nxtEval);

        double diff = nxtVal - curVal;
        ...
        if (accept) {
            ...
            if (betterEval(curEval, bestEval)) {
                best = cur;
                bestEval = curEval;
            }
        }
