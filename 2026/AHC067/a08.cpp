...
// 距離評価の重み。
// 大きくすると「対応スイッチから遠い扉」を好みやすくなる。
// ただし大きすぎると、本来重要な T の最大化が弱くなる。
const double DIST_WEIGHT = 0.0001;
...
Solution makeInitialSolution(const vector<pair<int, int>>& path) {
    ...
    for (int k = 0; k < gateCount; k++) {
        ...
        // 型 2k+1 は初期状態で閉じていて、スイッチ k を押すと開く。
...
long long calcDoorSwitchDistanceSum(const Solution& sol) {
    pair<int, int> swPos[MAXK];
    ...
    for (const auto& door : sol.doors) {
        int k = door.g / 2;

        int si = swPos[k].first;
        int sj = swPos[k].second;
        ...
        int dist1 = abs(si - i1) + abs(sj - j1);
        int dist2 = abs(si - i2) + abs(sj - j2);

        // 扉は 2 マスの間にあるため、近い方の端点との距離を使う。
        sum += min(dist1, dist2);
    }
...
Eval evaluateSolution(const Solution& sol) {
    int T = calcT(sol);

    if (T == 0) {
        return Eval{0, 0, -1e100};
    }

    long long distSum = calcDoorSwitchDistanceSum(sol);
    double value = (double)T + DIST_WEIGHT * (double)distSum;

    return Eval{T, distSum, value};
}
...
int main() {
    ...
    Eval curEval = evaluateSolution(cur);

    if (curEval.T == 0) {
        for (auto& e : cur.doors) {
            e.g = 0;
        }
        curEval = evaluateSolution(cur);
    }
    ...
    double startTemp = max(15.0, curEval.value * 0.30);
    double endTemp = 0.20;
    ...
    while (timer.elapsed() < TIME_LIMIT) {
        ...
        Eval nxtEval = evaluateSolution(nxt);

        if (nxtEval.T == 0) {
            continue;
        }

        double diff = nxtEval.value - curEval.value;
        ...
        if (accept) {
            ...
            if (curEval.value > bestEval.value) {
                best = cur;
                bestEval = curEval;
            }
        }
