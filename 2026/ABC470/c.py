...
def jikken():
  ...
  seg = SegmentTree(lambda a, b: a ^ b, lambda: 0, [0]*N)
...
def main():
  ...
  for _ in range(Q):
    ...
    if query[0] == 1:
      ...
      new_not_zero_list = []
      for x in not_zero_list:
        seg.set(x, max(seg.get(x) - 1, 0))
        if seg.get(x) > 0:
          new_not_zero_list.append(x)
        not_zero_list = new_not_zero_list  # 忘れてた
    xor = seg.all_prod()
    # ic(xor)
    print(xor)
