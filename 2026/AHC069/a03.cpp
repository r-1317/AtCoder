...
Region set_union_with_cell(const Region& cells, int cell) {
    ...
    auto it = lower_bound(cells.begin(), cells.end(), cell);
...
Region find_region(int p,
    ...
    for (int start = 0; start < SIZE; ++start) {
        ...
        // 境界線長が同じ場合は、開始地点から各マスまでの
        // マンハッタン距離の合計が小さい領域を優先する。
        vector<BeamState> beam{
            {4, 0, Region{start}, move(initial_frontier)}};
        ...
        for (int selected_count = 1; selected_count < p; ++selected_count) {
            ...
            for (const auto& state : beam) {
                for (const int cell : state.frontier) {
                    ...
                    const int new_perimeter =
                        state.perimeter + 4 - 2 * adjacent;
                    ...
                    for (const int next : NEIGHBORS[cell]) {
                        ...
                        auto it = lower_bound(new_frontier.begin(),
                    ...
                    ++serial;
                    candidates.push_back({new_perimeter, new_distance_sum,
                                          serial, move(new_cells),
                                          move(new_frontier)});
                }
            }
...
Placement find_best_placement(const array<unsigned char, SIZE>& grass,
    ...
    for (int k = 0; k < candidates_to_try; ++k) {
        ...
        Region destination =
            find_region(groups[i].p, i, grass, owner, &blocked);
...
int main() {
    ...
    for (int i = 0; i < M; ++i) {
        ...
        Placement placement = find_best_placement(grass, owner, groups, r);
        if (placement.found) {
