...
def find_best_placement(grid: List[List[int]], owner: List[List[int]], groups: List[GroupInfo], new_group: GroupInfo, R: float) -> Tuple[List[List[Tuple[int, int]]], List[Tuple[int, int]]]:
  ...
  隣接する4つのマスのうち2つが同じグループ(自分以外)に占有されている場合、そのマスには配置できない。
  ### 入力
  - grid: 芝生と池のグリッド
  - owner: 占有者のIDを保持するグリッド
  - groups: グループ情報のリスト
  ...
  def find_region(p: int, movable: int = -1, forbidden=None):
    ...
    for cell in range(size):
      if (not grass[cell]
          or (flat_owner[cell] != -1 and flat_owner[cell] != movable)
          or (forbidden is not None and cell in forbidden)):
        continue

      adjacent_owners = set()
      for nxt in neighbors[cell]:
        group_id = flat_owner[nxt]
        if group_id == -1 or group_id == movable:
          continue
        if group_id in adjacent_owners:
          break
        adjacent_owners.add(group_id)
      else:
        allowed[cell] = 1
...
def main():
  ...
  # 最後までに残っているグループの料金を加算する
  money += release_groups(groups, owner, 100001)
