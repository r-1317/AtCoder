...
def find_best_placement(grid: List[List[int]], owner: List[List[int]], groups: List[GroupInfo], new_group: GroupInfo, R: float) -> Tuple[List[List[Tuple[int, int]]], List[Tuple[int, int]]]:
  ...
  def find_region(p: int, movable: int = -1, forbidden=None):
    ...
    for start in range(size):
      ...
      beam = [(4, selected, frontier)]
      serial = 0
      for _ in range(1, p):
        candidates = []
        used = set()
        for perimeter, cells, edge in beam:
          for cell in sorted(edge):
            ...
            for nxt in neighbors[cell]:
              if allowed[nxt] and nxt not in new_cells:
                new_edge.add(nxt)
            serial += 1
            candidates.append((new_perimeter, serial, new_cells, frozenset(new_edge)))
        if not candidates:
          break
        beam = [(perimeter, cells, edge) for perimeter, _, cells, edge in heapq.nsmallest(BEAM_WIDTH, candidates, key=lambda state: (state[0], state[1]))]
      if beam and len(beam[0][1]) == p:
        best = min(beam, key=lambda state: state[0])[1]
