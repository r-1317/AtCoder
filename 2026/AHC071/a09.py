...
def main():
  ...
  これを繰り返す。
  ### 特殊な処理
  右端からEDGE個のマスは、右から順に見ていく。
  """
  for y in range(H - 1, -1, -1):  # 上から下に向かってレンガを配置していく
    for x in range(W):
      if x < W - EDGE:  # 右端からEDGE個のマス以外は通常の処理
        ...
        if hole_bb.is_set(x, y):  # 穴がある場合は、そこからレンガを置く
          ...
          for l in [1, 3, 5, 7, 9]:  # レンガの長さを短い方から試す
            if y == 0 and l > 3:# 最下層では幅5以上のレンガは置かない
              break
      ...
      else:  # 右端からEDGE個のマスは、右から順に見ていく
        # 55なら59, 56なら58, 57なら57, 58なら56, 59なら55
        x_rev = W - 1 - (x - (W - EDGE))
        if brick_bb.is_set(x_rev, y):  # すでにレンガが置かれている場合はスキップ
          continue
        if hole_bb.is_set(x_rev, y):  # 穴がある場合は、そこからレンガを置く
          max_hole_count = 0  # 覆える穴の最大数
          best_l = 1  # 覆える穴の最大数を得られる
          for l in [1, 3, 5, 7, 9]:  # レンガの長さを短い方から試す
            if y == 0 and l > 3:# 最下層では幅5以上のレンガは置かない
              break
            if x_rev - l + 1 < 0:  # レンガが盤面からはみ出す場合はスキップ
              continue
            # 覆う範囲にすでにレンガが置かれている場合はスキップ
            if any(brick_bb.is_set(x_rev - i, y) for i in range(l)):
              continue
            # レンガを置いたときに覆える穴の数を数える
            hole_count = sum(hole_bb.is_set(x_rev - i, y) for i in range(l))
            if hole_count > max_hole_count and hole_count / l >= 0.23:  # 覆える穴の数が最大で、かつ穴の割合がしきい値以上の場合
              max_hole_count = hole_count
              best_l = l
          # 最も穴を塞げるレンガを置く
          for i in range(best_l):
            brick_bb.set(x_rev - i, y)  # レンガを置く
          total_cost += cost_list[(best_l - 1) // 2]  # レンガのコストを加算
          ans_list.append((x_rev - best_l + 1, y, best_l))  # レンガの配置を記録
          # レンガの中心の下のマスに新たな穴を開ける
          center_x = x_rev - best_l // 2
          if y > 0:  # 下のマスが存在する場合のみ穴を開ける
            hole_bb.set(center_x, y - 1)
