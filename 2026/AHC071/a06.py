...
def main():
  ...
  """
  ### 方針
  層を左から見ていって、レンガがない場所から、そこから各長さのレンガを短い方から試す。
  ...
  for y in range(H - 1, -1, -1):  # 上から下に向かってレンガを配置していく
    for x in range(W):
      ...
      if best_l == -1:  # 覆える穴の数が最大で、かつ穴の割合がしきい値以上のレンガが見つからなかった場合はスキップ
        continue
      if max_hole_count == 1 and best_l > 1:  # 覆える穴の数が1個で、かつレンガの長さが1より大きい場合はスキップ
        continue
