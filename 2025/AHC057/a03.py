...
def get_nearest_points_at_time(point_coords: List[List[float]], velocities: List[List[float]], uf: UnionFind, current_time: int, time: int, point_id: int, num_points: int, L: int, K: int, M: int, large_components_count: int) -> Tuple[List[int], int]:
  N = len(point_coords)
    # x, y = point_coords[point_id]
  ...
  for i in range(N):
    ...
    # 距離がDIST以内ならその場で結合
    if distance <= DIST:
      if uf.size(point_id) < SIZE_LIMIT and uf.size(i) < SIZE_LIMIT and uf.size(point_id) + uf.size(i) >= SIZE_LIMIT:
        large_components_count += 1
      elif uf.size(point_id) >= SIZE_LIMIT and uf.size(i) >= SIZE_LIMIT:
        large_components_count -= 1
      uf.unite(point_id, i)
      continue
  ...
  return nearest_points, large_components_count
...
def main():
  ...
  for i in range((K-1)*M):  # 290回の繰り返し
    ...
    point_id_2 = -1
    for j in range(N):
      nearest_points, large_components_count = get_nearest_points_at_time(point_coords, velocities, uf, current_time, current_time, j, 1, L, K, M, large_components_count)  # 各点に対して最も近い1点を探す
    ...
    # 見つけた2点を結合
    if (point_id_1 != -1 and point_id_2 != -1) and (not uf.is_same(point_id_1, point_id_2) and uf.size(point_id_1) + uf.size(point_id_2) <= K) and (uf.size(point_id_1) + uf.size(point_id_2) < SIZE_LIMIT or large_components_count < M or (int(uf.size(point_id_1) < SIZE_LIMIT) + int(uf.size(point_id_2) < SIZE_LIMIT) == 1) or (uf.size(point_id_1) >= SIZE_LIMIT and uf.size(point_id_2) >= SIZE_LIMIT)):
  ...
  # 各点の、頂点までの距離の合計を表示（デバッグ用）
  total_distance = 0.0
  for i in range(N):
    root = uf.root(i)
    x1, y1 = point_coords[i]
    x2, y2 = point_coords[root]
    distance = distance_between_points((x1, y1), (x2, y2), L)
    total_distance += distance
  print("Total distance to roots:", total_distance, file=sys.stderr)
