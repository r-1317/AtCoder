...
# 中心(5000, 5000)からの距離が最も遠い都市を探す関数
def find_farthest(city_coord_list: list, is_grouped_list: list) -> int:
  max_dist = -1  # 最大距離
  farthest_city = None  # 中心から最も遠い都市の番号
  # グループに属さない都市の中で、中心からの距離が最も遠い都市を探す
  for i in range(N):  # 都市の数だけループ
    # グループに属している都市はスキップ
    if is_grouped_list[i]:
      continue
    dist = (city_coord_list[i][0] - 5000) ** 2 + (city_coord_list[i][1] - 5000) ** 2  #中心からの距離の2乗
    # 中心からの距離が最大の場合、都市を更新
    if max_dist < dist:
      max_dist = dist
      farthest_city = i
  # 中心からの距離が最大の都市が見つからなかった場合、エラーを返す
  # これはあり得ないはずだが、念のため
  if farthest_city is None:
    # ic(is_grouped_list)
    raise ValueError("No ungrouped city found.")
  return farthest_city  # 中心から最も遠い都市の番号を返す
...
def make_group(g: int, near_city_list: list, city_coord_list: list, is_grouped_list: list) -> list:
  ...
  # まずはグループの始点となる都市を選ぶ
  # 中心(5000, 5000)からの距離が最も遠い都市をグループの始点とする
  first_city = find_farthest(city_coord_list, is_grouped_list)  # 中心から最も遠い都市を選ぶ
