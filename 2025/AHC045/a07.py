...
# 中心(5000, 5000)からの距離が最も近い都市を探す関数
def find_farthest(city_coord_list: list, is_grouped_list: list) -> int:
  min_dist = float("inf")  # 最小距離
  nearest_city = None  # 中心から最も近い都市の番号
  # グループに属さない都市の中で、中心からの距離が最も近い都市を探す
  for i in range(N):  # 都市の数だけループ
    ...
    if dist < min_dist:
      min_dist = dist
      nearest_city = i
  # 中心からの距離が最小の都市が見つからなかった場合、エラーを返す
  # これはあり得ないはずだが、念のため
  if nearest_city is None:
    # ic(is_grouped_list)
    raise ValueError("No ungrouped city found.")
  return nearest_city  # 中心から最も近い都市の番号を返す
...
def make_group(g: int, near_city_list: list, city_coord_list: list, is_grouped_list: list) -> list:
  ...
  # まずはグループの始点となる都市を選ぶ
  # 中心(5000, 5000)からの距離が最も近い都市をグループの始点とする
  first_city = find_farthest(city_coord_list, is_grouped_list)  # 中心から最も近い都市を選ぶ
