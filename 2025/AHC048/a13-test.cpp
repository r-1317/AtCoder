...
int  well_size  = 4;              // 1 ウェル辺のマス数
int  wells_num  = N * N / (well_size * well_size);
...
int main(){
    ...
    for(int w=0; w<wells_num; ++w){
        for(int t=0; t<well_size*well_size/4; ++t){  // 1ウェルあたり1グラムは開けておく
            int k = randK(rng);
    ...
    int boost_limit = 899;  // 新たに絵の具を追加しないことを許可する閾値
    // 0から1の乱数
    uniform_real_distribution<double> rand01(0.0, 1.0);

    /* 目標 1000 色を順に処理 */
    double total_diff = 0.0;
    for(size_t target_index = 0; target_index < target_list.size(); ++target_index){
        const auto& target = target_list[target_index];
        int trans_count = 0;  // コストが同じ場合の遷移が行われた回数
        int j_start = 0;  // jの開始値
        if (target_index < boost_limit) {
            j_start = 1;
        }
        ...
        for(int wi=0; wi<wells_num; ++wi){
            for(int pk=0; pk<K; ++pk){
                for(int j=j_start; j<=2; ++j){
                    ...
                    double cost = D * (j - 1 + discard_cnt) + diff * 1e4;
                    if(cost <= best_cost){
                        // コストが同じならtrans_prob * (1 - 0.4*trans_count)の確率で遷移
                        if(cost == best_cost){
                            if(rand01(rng) > trans_prob * (1 - 0.4 * trans_count)){
                                continue;
                            }
                            else{
    ...
    /* 総コストを出力 */
    float total_score = 1 + D*(add_count - H) + 10000 * total_diff;
    cout << (int)round(total_score) << '\n';
    /* すべて出力 */
