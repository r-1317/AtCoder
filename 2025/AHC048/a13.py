...
def main():
  ...
  for target_index, target in enumerate(target_list):
    ...
    j_start = 0  # jの開始値
    j_start = 1 if target_index <= boost_limit else 0  # jの開始値
    ...
    for well_index in range(wells_num):
      ...
      for i, owm in enumerate(owm_list):
        ...
        for j in range(j_start, 3):
          ...
          # コストを計算
          tmp_cost = D*(j-1+discard_count) + diff*10**4
          # 最小コストを更新
          if tmp_cost <= min_tmp_cost:
            # コストが同じならtrans_prob * (1 - 0.4*trans_count)の確率で遷移
            if tmp_cost == min_tmp_cost:
              if random.random() > trans_prob * (1 - 0.4*trans_count):  # 遷移確率
                continue
              else:
                trans_count += 1  # 遷移が行われた回数をカウント
            min_tmp_cost = tmp_cost
