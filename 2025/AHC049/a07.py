  ...
  for i in range(max_x + 1):
    for j in range(max_y + 1):
      if not is_box_removed[i * N + j] and d_list[i][j] < max_weight:  # 取り除かれていないダンボール箱で、上に積めるもの
        score = vf06((i, j), w_list, d_list)  # 評価関数を計算
...
# 06の評価関数
def vf06(coord: Tuple[int, int], w_list: List[List[int]], d_list: List[List[int]]) -> int:
  x, y = coord
  w = w_list[x][y]  # 重さ
  d = d_list[x][y]  # 耐久値
  return d * manhattan_distance(coord)**2  # 耐久値 * 原点までのマンハッタン距離の二乗を評価関数とする
