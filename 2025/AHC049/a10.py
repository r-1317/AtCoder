...
# 可処分耐久値の最小値を計算する関数
def calc_min_hp(carrying_list: List[List[int]], takahashi: List[int]) -> int:
  dist = manhattan_distance(takahashi)  # 原点までのマンハッタン距離
  min_hp = 10**9
  tmp_wheight = 0  # 重さの合計
  for i in range(len(carrying_list)-1, -1, -1):
    hp = carrying_list[i][1] - tmp_wheight * dist  # 耐久値 - 重さ * 原点までの距離
    min_hp = min(min_hp, hp)  # 最小値を更新
    tmp_wheight += carrying_list[i][0]  # 重さを加算
  return min_hp  # 可処分耐久値の最小値を返す

# 寄り道できる位置の上に積めるダンボールのうち、最も原点の遠くにあるものを1つ選ぶ関数
def find_best_box(is_box_removed: List[bool], min_hp: int, takahashi: List[int], w_list: List[List[int]], d_list: List[List[int]]) -> Tuple[int, int]:
  best_coord = (-1, -1)
  best_score = -10**9
  max_x, max_y = takahashi
  for i in range(max_x + 1):
    for j in range(max_y + 1):
      if not is_box_removed[i * N + j] and w_list[i][j] * manhattan_distance((i, j)) < min_hp:  # 取り除かれていないダンボール箱で、上に積めるもの
        score = vf10((i, j), w_list, d_list)  # 評価関数を計算
...
def vf04(coord: Tuple[int, int], w_list: List[List[int]], d_list: List[List[int]]) -> float:
  ...
  w = w_list[x][y]  # 重さ
  d = d_list[x][y]  # 耐久値
  return w / d  # 耐久値/重さの比率を評価関数とする <- 間違い。実際は重さ/耐久値となっている。
# 05の評価関数
def vf05(coord: Tuple[int, int], w_list: List[List[int]], d_list: List[List[int]]) -> int:
  x, y = coord
  w = w_list[x][y]  # 重さ
  d = d_list[x][y]  # 耐久値
...
# 08の評価関数
def vf08(coord: Tuple[int, int], w_list: List[List[int]], d_list: List[List[int]]) -> int:
  x, y = coord
  w = w_list[x][y]  # 重さ
  d = d_list[x][y]  # 耐久値
  return w * manhattan_distance(coord)  # 重さ * 原点までのマンハッタン距離を評価関数とする

# 09の評価関数
def vf09(coord: Tuple[int, int], w_list: List[List[int]], d_list: List[List[int]]) -> int:
  x, y = coord
  w = w_list[x][y]  # 重さ
  d = d_list[x][y]  # 耐久値
...
def main():
  ...
  while remove_count < N**2 - 1:
    ...
    while target != (-1, -1):  # 積める箱が見つからなくなるまで
      ...
      min_hp = calc_min_hp(carrying_list, takahashi)  # 可処分耐久値の最小値を計算
      target = find_best_box(is_box_removed, min_hp, takahashi, w_list, d_list)
