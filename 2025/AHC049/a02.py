...
def find_best_box(d_list: List[List[int]], is_box_removed: List[bool], max_weight: int, takahashi: List[int]) -> Tuple[int, int]:
  best_coord = (-1, -1)
  best_distance = -1
  ...
  for i in range(max_x + 1):
    for j in range(max_y + 1):
      if not is_box_removed[i * N + j] and d_list[i][j] < max_weight:  # 取り除かれていないダンボール箱で、上に積めるもの
        ...
        if distance > best_distance:  # 原点からの距離が最も遠いものを選ぶ
