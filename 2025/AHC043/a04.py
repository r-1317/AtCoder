...
def main():
  ...
  # 現在の資金で建てられる最も長い通勤経路を求める
  commuter1, construction_queue = get_longest_path(money, home_list, workplace_list)
  income_count = len(construction_queue)  # 収入を得るまでの残りの建設数
  income = income_count - 1  # 収入

  # ic(construction_queue)

  # この時点での建設シミュレーションも行う
  money1, ans_list1 = construct(construction_queue[:], money, income, income_count, t)
  ic(money1)
  ...
  nearest_commuter = get_nearest_commuter(commuter1, home_list, workplace_list)

  # 家同士を結ぶ最短経路を求める
  tmp_construction_queue = get_path02(home_list[nearest_commuter][0], home_list[nearest_commuter][1], home_list[commuter1][0], home_list[commuter1][1], grid)
  construction_queue += tmp_construction_queue
  # 建設をグリッドに反映
  grid = reflect_construction(grid, tmp_construction_queue)
  # 職場同士を結ぶ最短経路を求める
  tmp_construction_queue += get_path02(workplace_list[nearest_commuter][0], workplace_list[nearest_commuter][1], workplace_list[commuter1][0], workplace_list[commuter1][1], grid)
  construction_queue += tmp_construction_queue
  # 建設をグリッドに反映
  # grid = reflect_construction(grid, tmp_construction_queue)  # 現在は不要

  # ic(construction_queue)

  # ターン毎の建設シミュレーション
  money2, ans_list2 = construct(construction_queue, money, income, income_count, t)
  ic(money2)

  # 2つの建設シミュレーションの結果のうち、資金が多い方を出力
  if money2 < money1:
