...
threshold = 16  # 武器の攻撃力がこれ以上の場合にのみ使用する
...
def best_weapon(owned_weapons: List[int], suitableness_matrix: List[List[int]], box_index: int) -> int:
  ...
  for wpn in owned_weapons:
    ...
    if power < threshold:  # 攻撃力が閾値未満の場合は使用しない
      continue
    if power > best_power:
      best_power = power
      best_wpn = wpn
...
def use_weapon(owned_weapons: List[int], weapon_index: int, box_index: int, weapon_hp_list: List[int], box_hp_list: List[int], suitableness_matrix: List[List[int]]) -> str:
  ...
  if attack_power < threshold and weapon_index > 0:  # 攻撃力が閾値未満の場合は使用しない
    print(f"武器{weapon_index}の攻撃力{attack_power}が閾値{threshold}未満のため、素手攻撃を行います。", file=sys.stderr)
    return f"-1 {box_index}"  # 素手攻撃コマンドを返す
...
def owned_best_pair(box_hp_list: List[int], weapon_hp_list: List[int], suitableness_matrix: List[List[int]], owned_weapons: List[int]) -> Tuple[int, int]:
  ...
  for box_index in range(N):
    ...
    for weapon_index in owned_weapons:
      ...
      if attack_power < threshold:  # 攻撃力が閾値未満の場合は使用しない
        continue
...
# 所持している武器で壊せる宝箱の中で、耐久値が最大の宝箱のインデックスを返す
def toughest_breakable_box(box_hp_list: List[int], weapon_hp_list: List[int], suitableness_matrix: List[List[int]], owned_weapons: List[int]) -> int:
  toughest_index = -1
  toughest_hp = -1
  for box_index in range(N):
    if box_hp_list[box_index] <= 0:
    ...
    can_break = False
    sum_attack_power = 0
    # 所持している武器をすべて使った場合の総攻撃力を計算
    for weapon_index in owned_weapons:
      attack_power = suitableness_matrix[weapon_index][box_index]
      if attack_power >= threshold:  # 攻撃力が閾値以上の場合のみ使用
        sum_attack_power += attack_power * weapon_hp_list[weapon_index]
      if sum_attack_power >= box_hp_list[box_index]:
        can_break = True
        break
    if can_break and box_hp_list[box_index] > toughest_hp:
      toughest_hp = box_hp_list[box_index]
      toughest_index = box_index
  return toughest_index
...
def main():
  ...
  sum_box_hp = sum(box_hp_list)  # 得点の計算用に全宝箱の耐久値の合計を取得
  turn = 0  # ターン数のカウント用変数。得点の計算に使用
  ...
  while open_count < N:
    turn += 1  # ターン数をインクリメント
    # 武器が残っている場合は、現在の手持ち武器で壊せる箱の中で耐久値が最大のものを攻撃する
    if owned_weapons:
      box_index = toughest_breakable_box(box_hp_list, weapon_hp_list, suitableness_matrix, owned_weapons)  # 耐久値が最大の壊せる宝箱のインデックスを取得
      # もし壊せる宝箱がなければ、最も攻撃力が高くなる組み合わせを選択して使用
      if box_index == -1:
        box_index, weapon_index = owned_best_pair(box_hp_list, weapon_hp_list, suitableness_matrix, owned_weapons)  # 最適な宝箱と武器の組み合わせを取得
      else:  # 壊せる宝箱がある場合は、その宝箱に対する最適な武器を使用
        weapon_index = best_weapon(owned_weapons, suitableness_matrix, box_index)  # その宝箱に対する最適な武器のインデックスを取得
      # 使用できる武器がある場合はその武器を使用
      if weapon_index != -2:
    ...
    # 武器が残っていない場合は、耐久値が最も低い宝箱を選択し、素手攻撃を行う
      ...
      command = use_weapon(owned_weapons, weapon_index, box_index, weapon_hp_list, box_hp_list, suitableness_matrix)
      print(command)
    # 武器を持っていながら、最大攻撃力が閾値未満の場合は、耐久値が最も低い宝箱を選択し、素手攻撃を行う
    if weapon_index == -2:
      box_index = weakest_box(box_hp_list)  # 耐久値が最も低い宝箱のインデックスを取得
