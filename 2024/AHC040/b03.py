...
def main():
  ...
  for i in range(t):
        ...
        for r in range(2):  # 回転の有無
          for d in range(2):  # 移動方向
            for b in range(-1, j):  # 移動先
              ...
              # stateに追加
              state_list[j+1].append([cost, False, box_list, state[3]+  [tmp_ans_list]])
        state[1] = True
        # 幅1なので、1つの状態を探索したら次に進む
        break
      state_list[j+1].sort(key = lambda x: x[0])
    ...
    # 実行時間が2900msを超える場合は、実行を中断する
    ic(time.time() - start_time)
    if 2.9 < time.time() - start_time:
