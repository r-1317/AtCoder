...
def main():
  ...
  state_list[0].append([0, 2*10**9, False, [[[0, 0, 10**9]], [[0, 0, 10**9]]], []])  # 長編の長さ, コスト, 探索済みか, box_list, ans_list
        ...
        for r in range(2):  # 回転の有無
          for d in range(2):  # 移動方向
            for b in range(-1, j):  # 移動先
              box_list = [[a[:] for a in b] for b in state[3]]
              # ans_list = [a[:] for a in state[4]]
              tmp_ans_list = [j, r, "L" if d else "U", b]
              box_list = add_rectangle(box_list, tmp_ans_list, rectangle_list[j], s)
              cost = calc_cost(box_list)
              max_side = max(box_list[0][0][0], box_list[1][0][0])
              occupancy = sum_size_list[j] / (box_list[0][0][0] * box_list[1][0][0])
              # stateに追加
              state_list[j+1].append([max_side, cost, False, box_list, state[4]+  [tmp_ans_list]])
        state[2] = True
        # 幅1なので、1つの状態を探索したら次に進む
        break
      state_list[j+1].sort(key = lambda x: (x[0], x[1]), reverse = False)
