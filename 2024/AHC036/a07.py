...
##### 路線に限りfind_pathを復活させる(新たなmain関数は作らない) #####
...
# 最寄りの駅を求める
def find_nearest_station(shortest_path_list, stations_list, pos):
  min_path = 10**9
  nearest = -1

  for station in stations_list:
    path = shortest_path_list[pos][station]  # その駅との最短経路
    if path is None:  # 経路が存在しない場合はスキップ
      continue
    if len(path) < min_path:
      min_path = len(path)
...
# 信号を制御する
def control_signal(a_list, b_list, p_index, l_b, type):# typeについて 0: 順方向, 1: 逆方向, 2: 中央(向きなし), 3: 1つだけ変更
  # 順方向の場合
  if type == 0:
  ...
  # 中央の場合(pがb_listの中心になる)
  elif type == 2:
    if l_b % 2 == 0:  # l_bが偶数の場合
      if p_index-l_b//2 < 0:  # indexが負になる場合 (おそらく存在しないが、念の為)
        b_list = a_list[:l_b]
        print(f"s {l_b} 0 0")
      elif len(a_list) < p_index+l_b//2:  # indexが配列Aの長さを超える場合
        b_list = a_list[-l_b:]
        print(f"s {l_b} {len(a_list)-l_b} 0")
      else:  # indexが正の場合
        b_list = a_list[p_index-l_b//2:p_index+l_b//2]
        print(f"s {l_b} {p_index-l_b//2} 0")
    else:  # l_bが奇数の場合
      if p_index-l_b//2 < 0:  # indexが負になる場合
        b_list = a_list[:l_b]
        print(f"s {l_b} 0 0")
      elif len(a_list) < p_index+l_b//2+1:  # indexが配列Aの長さを超える場合
        b_list = a_list[-l_b:]
        print(f"s {l_b} {len(a_list)-l_b} 0")
      else:  # indexが正の場合
        b_list = a_list[p_index-l_b//2:p_index+l_b//2+1]
        print(f"s {l_b} {p_index-l_b//2} 0")
  # 1つだけ変更する場合
  elif type == 3:
    b_list[0] = a_list[p_index]
    print(f"s 1 {p_index} 0")

  return b_list

# 配列Aの余った部分に、まだ入っていない都市を追加
def fill_a_list(g_list, p_list, a_list, a_set, l_a):
  prev_city = None  # 一つ前の都市は存在しない
  tmp_pos = (500, 500)  # 現在地
  city_count = len(a_set)  # 都市の数

  while city_count != 600:  # すべての都市が入るまで
    # 隣接リストの都市のうち、a_setに含まれていない都市を返す
    next_city = -1
    if prev_city is not None:  # 初回以外
      # 隣接リストを距離順にソート
      g_list[prev_city].sort(key=lambda x: dist(p_list[x], tmp_pos))
      for city in g_list[prev_city]:
        if not(city in a_set):
          next_city = city
          break

    # 次の都市がない場合
    if next_city == -1:
      # まだ入っていない都市すべての中で、最もユークリッド距離が近い都市を選択
      min_dist = 10**9
      for i in range(600):
        if not(i in a_set):
          d = dist(p_list[i], tmp_pos)
          if d < min_dist:
            min_dist = d
            next_city = i

    # 配列Aに追加
    a_list.append(next_city)
    a_set.add(next_city)
    city_count += 1
    tmp_pos = p_list[next_city]
    prev_city = next_city
...
# 隣接リストから最短経路を求める
def bfs_shortest_path(adj_matrix):
  n = len(adj_matrix)  # ノードの数を取得
  # 最短距離を格納する行列を無限大で初期化
  distances = [[float('inf')] * n for _ in range(n)]
  # 経路上の直前のノードを格納する行列を -1 で初期化
  predecessors = [[-1] * n for _ in range(n)]
  
  # 各ノードを開始点として幅優先探索を実行
  for start in range(n):
    distances[start][start] = 0  # 自分自身への距離は0
    queue = deque([start])  # 幅優先探索のためのキューを初期化

    while queue:
      node = queue.popleft()  # キューから現在のノードを取り出す

      for neighbor in range(n):
        if adj_matrix[node][neighbor] == 1 and distances[start][neighbor] == float('inf'):
          # エッジが存在し、まだ訪問していないノードの場合
          distances[start][neighbor] = distances[start][node] + 1
          predecessors[start][neighbor] = node
          queue.append(neighbor)  # 隣接ノードをキューに追加
  
  return distances, predecessors


# 直前の頂点をたどって最短経路を求める
# 出典: https://github.com/nkmk/python-snippets/blob/4375291ae06046bc18d7cd914b6dfac2d5a30f3a/notebook/scipy_shortest_path.py#L109-L121
...
def get_path_row(start, goal, pred_row):
  ...
  if i < 0:
    return None
  # path.append(i)  # 開始地点は含めない
  path = path[:index]
  return path[::-1]

# すべての都市間の最短経路を求める
def find_shortest_path(adj_list):
  dist_list, pred_list = bfs_shortest_path(adj_list)  # 最短距離と前の都市のリスト
  shortest_path_list = [[[] for _ in range(600)] for _ in range(600)]  # 最短経路リスト

  for start in range(600):
    for goal in range(600):
      shortest_path_list[start][goal] = get_path(start, goal, pred_list)  # 最短経路を求める

  return shortest_path_list


# 愚直な解法
def main_01(n, m, t, l_a, l_b, g_list, t_list, p_list, shortest_path_list):
  print("# 01を実行")
  # # 入力処理
  ...
  for pos_to in t_list:
    ...
    path_list += shortest_path_list[pos_from][pos_to]  # 最短経路を追加
    pos_from = pos_to
  ...
  a_list = fill_a_list(g_list, p_list, a_list, a_set, l_a)
  ...
  for p in path_list:
    if not(p in b_list):  # 配列Bに含まれていない場合
      p_index = a_list.index(p)
      b_list = control_signal(a_list, b_list, p_index, l_b, 2)
...
# 02の解法
def main_02(n, m, t, l_a, l_b, g_list, t_list, p_list, shortest_path_list):
  print("# 02を実行")
  # # 入力処理
  # n, m, t, l_a, l_b = map(int, input().split())  # n = 都市の数, m = 道路の数, t = 目的地の数, l_a = 配列Aの長さ, l_b = 配列Bの長さ
  # # n, t は常に600
  ...
  # east_line = shortest_path_list[center_station][east]
  # weast_line = shortest_path_list[center_station][weast]
  weast_line.reverse()
  ic(len(east_line))
  ic(len(weast_line))
  ...
  # north_line = shortest_path_list[center_station][north]
  # south_line = shortest_path_list[center_station][south]
  north_line.reverse()
  ic(len(north_line))
  ic(len(south_line))
  ...
  duplicate_count = len(a_list) - len(a_set)  # 重複している都市の数

  if duplicate_count + 600 <= l_a:  # 重複している都市の数と600を足しても、配列Aの長さを超えない場合
    # a_listに入っていない都市を追加
    a_list = fill_a_list(g_list, p_list, a_list, a_set, l_a)
    a_fill_type = 2
    print("# 効率よく配列Aを埋めることができた。")
  else:
    main_01(n, m, t, l_a, l_b, g_list, t_list, p_list, shortest_path_list)  # 入りきらない場合、01の関数に移行
    sys.exit()  # プログラムを終了
    ...
    for t in t_list:
      if len(a_list) == l_a:# 入りきらない場合、01の関数に移行
  ...
  for pos_to in t_list:
    # 直接、目的地に行く場合
    direct_path = shortest_path_list[pos_from][pos_to]  # 直接行く場合の経路
    if direct_path is None:  # たどり着けない場合、01の関数に移行
      main_01(n, m, t, l_a, l_b, g_list, t_list, p_list, shortest_path_list)
      sys.exit()  # プログラムを終了
    ...
    # 駅までの経路
    from_station_path = shortest_path_list[pos_from][from_station]  # 現在地から駅までの経路
    to_pos_to_path = shortest_path_list[to_station][pos_to]  # 駅から目的地までの経路
  ...
  # path_listに沿って移動、信号を制御
  for i, (p, line_type) in enumerate(path_list):
    # 配列Bに含まれていない場合
    if not(p in b_list):
      # バスを用いない場合
      if line_type == 0:
        p_index = a_list.index(p)
        if 0 < i < len(path_list) - 1:  # 最初または最後の要素でない場合
          if path_list[i-1][1] != 0 and path_list[i+1][1] != 0:  # 前後の要素がバスを使う場合
            b_list = control_signal(a_list, b_list, p_index, l_b, 3)  # 次の都市の信号一つだけを追加
          else:
            b_list = control_signal(a_list, b_list, p_index, l_b, a_fill_type)
        else:
          b_list = control_signal(a_list, b_list, p_index, l_b, a_fill_type)
      # 東西線を順方向に使う場合
      elif line_type == 1:
        p_index = 東西線.index(p)
        b_list = control_signal(a_list, b_list, p_index, l_b, 0)
      # 東西線を逆方向に使う場合
      elif line_type == 2:
        p_index = 東西線.index(p)
        b_list = control_signal(a_list, b_list, p_index, l_b, 1)
      # 南北線を順方向に使う場合
      elif line_type == 3:
...
def main():
  ...
  shortest_path_list = find_shortest_path(adj_list)  # すべての都市間の最短経路を求める

  ic(shortest_path_list[0][1])

  # sum_len = 0
  # for i in range(600):
  #   for j in range(600):
  #     sum_len += len(shortest_path_list[i][j])

  # ic(sum_len/600**2)  # 平均13前後
  ...
  # 東線 = shortest_path_list[east][center_station]
  # 西線 = shortest_path_list[weast][center_station]
  # 南線 = shortest_path_list[south][center_station]
  # 北線 = shortest_path_list[north][center_station]
  # 北西線 = shortest_path_list[north_weast][center_station]
  # 南東線 = shortest_path_list[south_east][center_station]
  # 北東線 = shortest_path_list[north_east][center_station]
  # 南西線 = shortest_path_list[south_weast][center_station]
  ...
  if duplicate_count + 600 <= l_a:  # 重複している都市の数と600を足しても、配列Aの長さを超えない場合
    ...
    main_02(n, m, t, l_a, l_b, g_list, t_list, p_list, shortest_path_list)  # 入りきらない場合、02の関数に移行
    sys.exit()  # プログラムを終了
  ...
  for pos_to in t_list:
    # 直接、目的地に行く場合
    direct_path = shortest_path_list[pos_from][pos_to]
    if direct_path is None:  # たどり着けない場合、02の関数に移行
      main_02(n, m, t, l_a, l_b, g_list, t_list, p_list, shortest_path_list)
      sys.exit()  # プログラムを終了
    direct_cost = len(direct_path)  # 直接行く場合の信号操作のコスト
    # ic(direct_path)
    # ic(direct_cost)
    ...
    # バス路線を経由する場合
    from_station = find_nearest_station(shortest_path_list, stations_list, pos_from)  # 現在地から最も近い駅
    to_station = find_nearest_station(shortest_path_list, stations_list, pos_to)  # 目的地から最も近い駅
    if from_station == -1 or to_station == -1:  # 駅が見つからない場合、02の関数に移行
      main_02(n, m, t, l_a, l_b, g_list, t_list, p_list, shortest_path_list)
      sys.exit()

    # 駅までの経路
    from_station_path = shortest_path_list[pos_from][from_station]  # 現在地から最も近い駅までの経路
    to_pos_to_path = shortest_path_list[to_station][pos_to]  # 目的地から最も近い駅までの経路
...
if __name__ == "__main__":
  ...
  ic(end_time - start_time)  # 処理時間を出力