...
# すべての都市間の最短経路を求める
def find_shortest_path(adj_arr):
  dist_arr, pred_arr = shortest_path(adj_arr, return_predecessors=True, directed=False, unweighted=True, method="FW")  # 最短距離と前の都市のリスト
  shortest_path_list = [[[] for _ in range(600)] for _ in range(600)]  # 最短経路リスト

  for start in range(600):
    for goal in range(600):
      shortest_path_list[start][goal] = get_path(start, goal, pred_arr)  # 最短経路を求める
...
def main():
  ...
  adj_arr = np.array(adj_list)  # 隣接行列

  shortest_path_list = find_shortest_path(adj_arr)  # 最短経路リスト

  ic(shortest_path_list[0][2])
  # sum_len = 0
  # for i in range(600):
  #   for j in range(600):
